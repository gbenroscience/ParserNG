# ParserNG
<b>ParserNG</b> is a powerful open-source math tool that parses and evaluates algebraic expressions and also knows how to handle a lot of mathematical expressions. Its latest release on mavn-central (version 1.1.4) can be used to plot 2D graphs(Geometric plots and function plots) It works seamlessly on all Java platforms.<br><br>
...
ParserNG v1.1.4 breaks the barrier in fully featured math parser frequencies!
With various other optimizations, v1.1.4 comes with inner loop optimizations which uses a blazing fast, post-fix style algorithm to quickly evaluate inner brackets with less checks and less string manipulations.
This obviously will make graphing and other iterative tasks super responsive.

ParserNG v1.1.4 is an extremely feature rich math tool which also doubles as (arguably) the fastest pure Java expression evaluator on the planet.
...
ParserNG 1.1.4 features strength reduction, constant folding and execution frame(array) based args passing(in contrast to Map based) to ensure O(1) complexity in passage of args to the evaluation stage.
...
```
java -jar parser-ng-1.1.4.jar  1+1
2.0
```
Or as logical parser
```
java -jar parser-ng-1.1.4.jar -l true and true
true
java -jar parser-ng-1.1.4.jar -l "2 == (4-2)"
true
```
You can get help by 
```
java -jar parser-ng-1.1.4.jar  -h
  ParserNG 1.1.4 math.Main
...
```
java -jar parser-ng-1.1.4.jar  -h -v
```
you can list functions:
```
java -jar parser-ng-1.1.4.jar  help
...
```
java -jar parser-ng-1.1.4.jar  -l help
...
```
  ...
  java -jar parser-ng-1.1.4.jar "1+1
    ...
    2.0
    4.0
  java -jar parser-ng-1.1.4.jar -t "1+1
                                    +2+2"
    6.0
  java -jar parser-ng-1.1.4.jar -i  1+1
    nothing, will expect manual output, and calculate line by line
  java -jar parser-ng-1.1.4.jar -i -t  1+1
    nothing, will expect manual output and calcualte it all as one expression
  echo 2+2 | java -jar parser-ng-1.1.4.jar  1+1
    2.0
  echo "1+1 
        +2+2 | java -jar parser-ng-1.1.4.jar -i
    2.0
    4.0
  echo "1+1 
        +2+2 | java -jar parser-ng-1.1.4.jar -i -t
    6.0
  java -cp parser-ng-1.1.4.jar parser.cmd.ParserCmd "1+1
    will ask for manual imput en evaluate per line
  echo "1+1 
        +2+2 | java -cp parser-ng-1.1.4.jar parser.cmd.ParserCmd 2>/dev/null
    2.0
    4.0
  java -cp parser-ng-1.1.4.jar parser.MathExpression "1+1
                                                      +2+2"
    6.0
  java -cp parser-ng-1.1.4.jar parser.LogicalExpression "true or false"
    true
...
Version 1.1.4 of ParserNG allows you to quickly compute the eigenvalues of a Matrix.
...
As of ParserNG 1.1.4 also, eigenvector computations have been added, do:
Do:
```Java
