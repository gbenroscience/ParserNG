...
ParserNG 2.0.0 features strength reduction, constant folding and execution frame(array) based args passing(in contrast to Map based) to ensure O(1) complexity in passage of args to the evaluation stage.
...
```
java -jar parser-ng-2.0.0.jar  1+1
2.0
```
Or as logical parser
```
java -jar parser-ng-2.0.0.jar -l true and true
true
java -jar parser-ng-2.0.0.jar -l "2 == (4-2)"
true
```
You can get help by 
```
java -jar parser-ng-2.0.0.jar  -h
  ParserNG 2.0.0 math.Main
...
```
java -jar parser-ng-2.0.0.jar  -h -v
```
...
```
java -jar parser-ng-2.0.0.jar  -l help
...
```
  ...
  java -jar parser-ng-2.0.0.jar "1+1
    ...
    2.0
    4.0
  java -jar parser-ng-2.0.0.jar -t "1+1
                                    +2+2"
    6.0
  java -jar parser-ng-2.0.0.jar -i  1+1
    nothing, will expect manual output, and calculate line by line
  java -jar parser-ng-2.0.0.jar -i -t  1+1
    nothing, will expect manual output and calcualte it all as one expression
  echo 2+2 | java -jar parser-ng-2.0.0.jar  1+1
    2.0
  echo "1+1 
        +2+2 | java -jar parser-ng-2.0.0.jar -i
    2.0
    4.0
  echo "1+1 
        +2+2 | java -jar parser-ng-2.0.0.jar -i -t
    6.0
  java -cp parser-ng-2.0.0.jar parser.cmd.ParserCmd "1+1
    will ask for manual imput en evaluate per line
  echo "1+1 
        +2+2 | java -cp parser-ng-2.0.0.jar parser.cmd.ParserCmd 2>/dev/null
    2.0
    4.0
  java -cp parser-ng-2.0.0.jar parser.MathExpression "1+1
                                                      +2+2"
    6.0
  java -cp parser-ng-2.0.0.jar parser.LogicalExpression "true or false"
    true
...
Version 2.0.0 of ParserNG allows you to quickly compute the eigenvalues of a Matrix.
...
As of ParserNG 2.0.0 also, eigenvector computations have been added, do:
Do:
```Java
