...
public class DerivativeOld {
     ...
     *
     */
    DerivativeOld(String expression) throws Exception {
        String mathExpr = "";
...
//the anonymous function to be differentiated: e.g.diff(@(p)(3*p^3+2*p^2-8*p+1),1)
            ...
            if (p.result == ParserResult.VALID) {
                ...
                if (p.isGradEval()) {
                    ...
                    for (int i = 1; i <= orderOfDiff; i++) {
                        DerivativeOld derivative = new DerivativeOld(expr);
    ...
    public static void main(String args[]) {
        ...
        DerivativeOld d;
        try {
            d = new DerivativeOld(f);
            System.out.println("diff: " + f + " = " + d.differentiate());
        } catch (Exception ex) {
            Logger.getLogger(DerivativeOld.class.getName()).log(Level.SEVERE, null, ex);
        }

        f = "diff(@(x)10*x^5,3)";
        try {
            MathExpression.EvalResult ev = DerivativeOld.eval(f);
...
//8x.sin(x^2)+8x^3.cos(x^2)-1
            ...
            MathExpression.EvalResult ev = DerivativeOld.eval(expression);
            t = System.nanoTime() - t;
