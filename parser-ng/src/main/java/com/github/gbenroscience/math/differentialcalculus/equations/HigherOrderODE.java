 ...
 *
 * The caller supplies only the top-order derivative, f(t, Y) where Y = (y, y',
 * ..., y^(n-1)), and an initial state y0 = (y(t0), y'(t0), ...) whose length
 * fixes the order n. CompanionSystemHandles builds the full companion
 * first-order system from that single handle, and VectorODE drives it — the
 * order-n reduction is invisible to the caller, exactly like the manual
 * reduction step is invisible in the diffeqnHO(...) surface syntax.
 *
...
public class HigherOrderODE {
    ...
    // ------------------------------------------------------------------
    public static double executeTurboODEHO(MethodHandle topDerivative,
            int tSlot,
            int ySlotStart,
            int frameSize,
            double t0,
     ...
     * @throws Throwable
     */
    public static double[][] executeTurboODEPathHO(MethodHandle topDerivative,
            int tSlot,
            int ySlotStart,
            int frameSize,
            double t0,
