...
public class Utilities {
    ...
    /**
     *
     * @param data An ArrayList of Strings. return the Strings in the List
    ...
    /**
     *
     * @param value The item to check Always double check in the scanner where
     * this item exists that the next token to <code>value</code> is not an open
     * bracket if value is a valid variable name.
    ...
    /**
     * Simplifies portions of math code that involve products or quotients of
     * -1.
     *
                ...
                if (list.get(i).equals("-1") || list.get(i).equals("-1.0")) {
                    ...
                    if (isMulOrDiv(list.get(i - 1))) {
                        ...
                        while (true) {
                            ...
                            else if (isClosingCircBracket(list.get(j))) {
                            ...
                            //...,(,(......)....,*,-1
                            else if (isOpeningCircBracket(list.get(j))) {
                                list.add(j + 1, "-");
                                list.remove(i);
                                i -= 1;
                                break;
     ...
     *
     * This method seeks to apply the algebraic effect of adding or subtracting
     * or multiplying zeroes to a math expression. e.g. 3*x*0+4*1 becomes 4*1.
     *
     */
     ...
     *
     * Method that recursively applies the effect of zero at its point of
     * occurrence to a math expression.
     *
     */
    public static void simplifyZeroesAt(int index, ArrayList<String> list) {
        ...
        if (list.get(index).equals("0") || list.get(index).equals("0.0")) {
            //(,0,+|-
            if (isOpeningCircBracket(list.get(index - 1)) && isPlusOrMinus(list.get(index + 1))) {
            ...
            else if (isOpeningCircBracket(list.get(index - 1)) && isClosingCircBracket(list.get(index + 1)) && !isMethodName(list.get(index - 2))) {
                list.remove(index + 1);
                list.remove(index - 1);
                --index;
            ...
            //0,*|/
                ...
                //0,*,num|var  
                if (isNumber(list.get(index + 2)) || (isVariableString(list.get(index + 2)) && !isOpeningCircBracket(list.get(index + 3)))) {
                    list.subList(index + 1, index + 3).clear();
                }//end if
                //0,*,sin,(,....,)
                else if (isVariableString(list.get(index + 2)) && isOpeningCircBracket(list.get(index + 3))) {
                ...
                //0,*,(,....,)
                else if (isOpeningCircBracket(list.get(index + 2))) {
            ...
            //...*,0...
                ...
                if (isNumber(list.get(index - 2)) || isVariableString(list.get(index - 2))) {
                    ...
                    else if (isOpeningCircBracket(list.get(index - 3))) {
                        list.subList(index - 2, index).clear();
                        index -= 2;
                    }
                ...
                //..,),*,0,
                else if (isClosingCircBracket(list.get(index - 2))) {
                    ...
                    if (isMethodName(list.get(open - 1))) {
                        ...
                        else if (isOpeningCircBracket(list.get(open - 2))) {
                    ...
                    //(,....,),*,0
                        ...
                        else if (isOpeningCircBracket(list.get(open - 1))) {
                            List temp = list.subList(open - 1, index);
                            int sz = temp.size();
                            temp.clear();
                            index -= sz;
     ...
     *
     * Seeks to calculate the effect of multiplying 1 with quantities in the
     * list...e.g 1*(....) becomes (.....), 1*sin(...) becomes sin(...)
     */
    ...
    /**
     * Checks for the pattern.. ...+,(,.......,)+|- and removes the bracket..if
     * possible.
     *
            ...
            try {
                if (isClosingCircBracket(scan.get(i))) {
                    ...
                    while (j < close) {
                        ...
                        //...,(,num,*|/,
                        else if (isNumber(scan.get(j)) && isOpeningCircBracket(scan.get(j - 1)) && isMulOrDiv(scan.get(j + 1))) {
                        ...
                        //...,*|/,num,),...
                        else if (isNumber(scan.get(j)) && isMulOrDiv(scan.get(j - 1)) && isClosingCircBracket(scan.get(j + 1))) {
                            if (scan.get(j - 1).equals("*")) {
                                value *= Double.parseDouble(scan.get(j));
                            } else {
                                value /= Double.parseDouble(scan.get(j));
    ...
    /**
     * Condenses numbers and variables to be added within a bracket to the end
     * of the bracket. For instance,(3+x+sin(3*x)+5-7) becomes (sin(3*x)+3+x-2)
     *
     * @param scan The scanner output.
     */
            ...
            try {
                if (isClosingCircBracket(scan.get(i))) {
                    ...
                    while (j < close) {
                        ...
                        //...,+|-,num,),...
                        else if (isNumber(scan.get(j)) && isPlusOrMinus(scan.get(j - 1)) && isClosingCircBracket(scan.get(j + 1))) {
                            if (scan.get(j - 1).equals("-")) {
                                value += -1 * Double.parseDouble(scan.get(j));
                            } else {
                                value += Double.parseDouble(scan.get(j));
