...
/**
 * ParserNG's single entry point for the diffeqn-family runtime: given the
 * full postfix of a diffeqn/diffeqnPath/diffeqnHO/diffeqnPathHO call,
 * routes end to end — argument parsing, equation isolation, coefficient
 * extraction, canonical-frame compilation, solver dispatch — and returns
 * the result in exactly the shape the calling convention promises.
 *
 * <h2>Calling convention (current)</h2>
 * The first argument to every one of the four entry points is the raw
 * equation itself, already rearranged to {@code LHS-RHS} with the {@code =}
 * sign omitted (implicitly {@code = 0}) — not a lambda. {@code y[0]} is the
 * state itself, {@code y[k]} its kth derivative, up through {@code y[n]} —
 * the term this class's coefficient resolver isolates and divides out.
 *
 * <h2>Canonical frame</h2>
 * Every solver call in this class uses the SAME canonical layout: slot 0 =
 * t, slots {@code 1..order} = {@code y[0..order-1]} — contiguous and
 * ascending by construction, regardless of what real, possibly-scattered
 * slots ParserNG's registry actually assigned to those names in the source
 * text. See {@link CanonicalFrame}.
 *
 * <h2>Analytic Jacobian for IMPLICIT_EULER</h2>
 * Built directly here rather than through {@code ExprNodeAnalyticJacobian}.
 * For the scalar case (order 1, {@code diffeqn}/{@code diffeqnPath}) it's a
 * single AD evaluator. For the higher-order case, the companion system's
 * shift rows ({@code dY_i/dt = Y_(i+1)}) have a trivially known derivative
 * (1 for the one matching column, 0 elsewhere) that needs no AD evaluator
 * at all — routing them through one anyway would require constructing an
 * {@code ExprNode} leaf for a state component that, in the sparse-equation
 * case, may have no real frame slot to build one from. Only the genuinely
 * compiled top row goes through {@code ExprNodeAutoDiffEvaluator}; the
 * trivial rows are filled directly.
 *
 ...
 * term. That case is not wired up: {@link #buildJacobianIfNeeded} returns
 * null for it (falls back to the solver's own finite-difference Jacobian)
 * rather than failing the call outright.
 *
 * <h2>Wired coefficient resolver</h2>
 * {@link CoefficientExtractor} is the real {@link EquationCoefficientResolver}
 * implementation — term splitting, linearity checking on the top-order term
 * only, coefficient division, and the canonical/real frame mapping are all
 * live. {@link #solve(MathExpression)} is the one-line entry point most
 * callers actually want: given an already-parsed diffeqn-family {@code
...
public final class EquationRuntime {
    ...
    /**
     * One-call convenience: parses, isolates, extracts, compiles, and
     * solves in a single line, using the real {@link CoefficientExtractor}.
     * Equivalent to:
     * <pre>{@code
     * new EquationRuntime(CoefficientExtractor::resolve).execute(me.getCachedPostfix())
     * }</pre>
     * This is the method to wire into {@code MathExpression}'s own {@code
     * solve()}/{@code solveGeneric()} — e.g. a one-line delegation:
     * {@code public Object solve() { return EquationRuntime.solve(this); } }
     * — so a ParserNG user only ever writes {@code new MathExpression(call).solve()}.
     *
     ...
     * either *_PATH variant — exactly the shape the calling convention
     * promises for that call kind.
     *
     * @param fullCallPostfix the WHOLE parsed postfix, with the
     *                        diffeqn-family call as its last token (the
     *                        postfix root) — e.g. {@code
     *                        me.getCachedPostfix()} for a call parsed as
    ...
    public Object execute(Token[] fullCallPostfix) {
        ...
        ODEFunction fn = new FrameRemappingODEFunction(resolved.topDerivativeRealFrame, frame);
    ...
    private DifferentialEquations.JacobianStrategy buildJacobianIfNeeded(
        ...
        if (call.method != DifferentialEquations.ODESolverMethod.IMPLICIT_EULER) {
            return null;
        }
     ...
     * complete pipeline, not just up to the isolation step. Also fixes a
     * bug the extractor itself would otherwise have caught: the equation's
     * highest term is {@code y[4]}, so its order is 4 and {@code y0} needs
     * 4 components (the earlier {@code (1, 0, 0, 0, 0)} had 5, implying
     * order 5 — {@code y[5]} — which never appears in the equation at all).
     * @param args
     */
