...
//the maximum finder algorithm begins
    ...
    /**
     *
     * @return displays the output of the sort method as the sorting process
     * proceeds.
     */
    ...
    /**
     *
     * @return sorts a number set and returns the result as a string of comma
     * separated values sorted in ascending order
     */
    public String sort() {
        List<String> sort = new ArrayList<String>();
...
//check if the random command came with only one argument
    ...
    public String evaluateUserDefinedFunction(String fName) throws ClassNotFoundException {
        ...
        for (int i = 0; i < sz; i++) {
            vars[i] = com.github.gbenroscience.parser.Number.fastParseDouble(data.get(i));
        }//end for loop
