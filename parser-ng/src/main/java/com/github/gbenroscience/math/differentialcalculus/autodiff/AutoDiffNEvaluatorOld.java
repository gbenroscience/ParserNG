 ...
 */
public class AutoDiffNEvaluatorOld implements Cloneable {

    @Override
    protected AutoDiffNEvaluatorOld clone() throws CloneNotSupportedException {
        return new AutoDiffNEvaluatorOld(formatTokens(this.rpnTokens), opcodes.clone(), constants.clone(), maxOrder, maxStackSize);
    }
    ...
    public AutoDiffNEvaluatorOld(MathExpression me) {
        this(me, Derivative.MAX_ORDER);
    }

    private AutoDiffNEvaluatorOld(Token[] rpnTokens, byte[] opcodes, double[] constants, int maxOrder, int maxStackSize) {
    ...
    public AutoDiffNEvaluatorOld(MathExpression me, int maxOrder) {
     ...
     * reused across different {@link AutoDiffNEvaluatorOld} instances -- without
    ...
    private static void test(String expr, String var, int order, double evalPoint) {
        ...
        AutoDiffNEvaluatorOld ad = new AutoDiffNEvaluatorOld(me, 10);
