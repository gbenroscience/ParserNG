 ...
 * off a diffeqn/diffeqnPath/diffeqnHO/diffeqnPathHO call — t0/tEnd/h/method/
 * points from {@link Token#getRawArgs()} text (the pragmatic path settled on
 * over re-deriving the same information structurally, per the explicit "if
 * too difficult, get it from token.getRawArgs()" fallback), but y0 from the
 * real compiled {@code Token}, since a bracketed vector literal like
 * {@code (1, 0, 0, 0, 0)} compiles to a single {@code MATRIX}-kind token
 * named {@code anonN} rather than staying literal text — re-splitting
 * rawArgs text on commas breaks the moment y0 contains any expression with
 * its own nested comma (a function call, another vector), and doesn't
 * reflect how ParserNG actually represents it. The real values are read via
 ...
 * PostfixArgumentIsolator} and {@link EquationCoefficientResolver} — see
 * the latter's javadoc for why it's the one open dependency in this
 * pipeline.
 *
 ...
 * </pre>
 * h and method are optional per the calling convention; when omitted this
 * class applies a documented default ({@link #DEFAULT_H}, {@link
 * #DEFAULT_METHOD}) rather than silently guessing something else — a caller
 * who cares about the exact solver behavior should always pass both.
 */
public final class DiffEqnArgParser {
    ...
    public static final DifferentialEquations.ODESolverMethod DEFAULT_METHOD =
            DifferentialEquations.ODESolverMethod.RK4;
     ...
     *                        postfix root) — needed, not just the call
     ...
     * bracketed vector literal compiles to a single {@code MATRIX}-kind
     * token named {@code anonN}: its real values are read via {@code
     * FunctionManager.lookUp(name).getMatrix().getFlatArray()}. A bare
     * scalar compiles to a single {@code NUMBER}-kind token: its value is
     * read directly off the token. Anything else falls back to splitting
     * rawText on commas — a defensive last resort, not the primary path.
     */
    ...
    /**
     * Last-resort fallback for the rare case y0's isolated token isn't a
     * plain MATRIX or NUMBER — parses a bare scalar ("1", "1.0") or a
     * parenthesized/bracketed comma-separated LITERAL vector ("(1, 0)",
     * "[1, 0, 0]"). Does not handle a vector containing expressions with
     * their own nested commas; {@link #resolveY0} should always have
     * already handled that case via the real MATRIX token before this runs.
     */
    ...
    public static DifferentialEquations.ODESolverMethod parseMethod(String raw) {
        ...
        switch (cleaned.toLowerCase()) {
            case "euler":
                return DifferentialEquations.ODESolverMethod.EULER;
            case "rk4":
                return DifferentialEquations.ODESolverMethod.RK4;
            case "rk45":
                return DifferentialEquations.ODESolverMethod.RK45_DORMAND_PRINCE;
            case "implicit_euler":
                return DifferentialEquations.ODESolverMethod.IMPLICIT_EULER;
            default:
                throw new IllegalArgumentException(
