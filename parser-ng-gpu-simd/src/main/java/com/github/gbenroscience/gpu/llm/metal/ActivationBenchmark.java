...
/**
 * Metal counterpart of {@code com.github.gbenroscience.gpu.llm.cuda.ActivationBenchmark}.
 * Same tests, same tolerance, same benchmark structure -- that class's
 * javadoc (why three timing numbers, why the tolerance isn't zero)
 * applies unchanged here. The only things that changed are mechanical:
 * device buffers are {@link MetalBuffer} values instead of {@code long}
 * CUdeviceptr, and "does the kernel actually run" is guaranteed by
 * {@link LlamaLayer}'s per-dispatch {@code waitUntilCompleted} (see that
 * class's javadoc) rather than an explicit {@code LlamaLayer.finish}
 * call -- {@code finish} is still called here where the CUDA source
 * does, for structural parity and because it stays meaningful API (see
 * its own javadoc) even though it is a no-op on this backend.
 *
 * Run with (adjust classpath):
 *   java --enable-preview -cp &lt;your-classpath&gt; com.github.gbenroscience.gpu.llm.metal.ActivationBenchmark
 */
public final class ActivationBenchmark {
        ...
        try {
            ...
            if (!allPassed) {
                ...
                System.err.println("One or more correctness checks FAILED -- fix before trusting benchmark numbers "
                        + "or any real model output from this Metal port.");
                System.exit(1);
            }
    ...
    private static float[] gpuReference(GpuContext ctx, Activation act, float[] gate, float[] up) throws Throwable {
        int len = gate.length;
        MetalBuffer gateDevice = LlamaLayer.uploadFloats(ctx, gate);
        MetalBuffer upDevice = (up != null) ? LlamaLayer.uploadFloats(ctx, up) : MetalBuffer.NULL;
        MetalBuffer outDevice = LlamaLayer.allocFloats(ctx, len);
        try {
    ...
    private static void dispatch(GpuContext ctx, Activation act, MetalBuffer gateDevice, MetalBuffer upDevice, MetalBuffer outDevice, int len) throws Throwable {
        switch (act) {
