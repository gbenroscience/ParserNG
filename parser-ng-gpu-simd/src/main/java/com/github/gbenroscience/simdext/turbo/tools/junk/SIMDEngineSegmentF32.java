 ...
 *
 */
public class SIMDEngineSegmentF32 extends VectorTurboEvaluator {
    ...
    public SIMDEngineSegmentF32(MathExpression me) throws Throwable {
        super(me);
    }

    public SIMDEngineSegmentF32(MathExpression me, int numWorkers) throws Throwable {
        super(me, numWorkers);
    }

    public static final SIMDEngineSegmentF32.SIMDVectorCompositeExpression getEvaluator(MathExpression me) throws Throwable {
        return (SIMDEngineSegmentF32.SIMDVectorCompositeExpression) new SIMDEngineSegmentF32(me).compile();
    }

    public static final SIMDEngineSegmentF32.SIMDVectorCompositeExpression getEvaluator(String expr) throws Throwable {
        return (SIMDEngineSegmentF32.SIMDVectorCompositeExpression) new SIMDEngineSegmentF32(new MathExpression(expr)).compile();
    }

    public static final SIMDEngineSegmentF32.SIMDVectorCompositeExpression getEvaluator(MathExpression me, int numWorkers) throws Throwable {
        return (SIMDEngineSegmentF32.SIMDVectorCompositeExpression) new SIMDEngineSegmentF32(me, numWorkers).compile();
    }

    public static final SIMDEngineSegmentF32.SIMDVectorCompositeExpression getEvaluator(String expr, int numWorkers) throws Throwable {
        return (SIMDEngineSegmentF32.SIMDVectorCompositeExpression) new SIMDEngineSegmentF32(new MathExpression(expr), numWorkers).compile();
    }
    ...
    public final class SIMDVectorCompositeExpression extends VectorTurboEvaluator.BatchedVectorCompositeExpression implements AutoCloseable {
         ...
         * {@code Float4Vector}'s data buffer), writing results directly into
        ...
        private static final class EvaluationContext {
            ...
            EvaluationContext(int maxStackDepth, int blockSize, int varCount) {
                ...
                segmentBlockVars = new float[varCount][blockSize];
        ...
        private void applyBulkInternal(MemorySegment variables, EvaluationContext ctx, long dataSize, MemorySegment output, long startIdx, long length) {
            ...
            for (long blockStart = startIdx; blockStart < endIdx; blockStart += BLOCK_SIZE) {
                ...
                for (int v = 0; v < varCount; v++) {
                    long srcOffsetBytes = ((v * dataSize) + blockStart) * ELEMENT_BYTES;
                    MemorySegment.copy(variables, srcOffsetBytes, ctx.segmentBlockVarsMem[v], 0L, currentBlockSize * ELEMENT_BYTES);
                }
                ...
                long destOffsetBytes = blockStart * ELEMENT_BYTES;
                MemorySegment.copy(MemorySegment.ofArray(ctx.stackArrays[0]), ctx.stackOffsets[0] * ELEMENT_BYTES, output, destOffsetBytes, currentBlockSize * ELEMENT_BYTES);
            }
        }
         ...
         * {@code blockStart * ELEMENT_BYTES} rather than from a computed offset within one
        ...
        private void applyBulkInternal(MemorySegment[] variables, EvaluationContext ctx, long dataSize, MemorySegment output, long startIdx, long length) {
            ...
            for (long blockStart = startIdx; blockStart < endIdx; blockStart += BLOCK_SIZE) {
                ...
                // FloatVector.fromMemorySegment / MemorySegment.getAtIndex. A variable
        ...
        private void materialize(EvaluationContext ctx, int targetSp, int n) {
            if (ctx.stackIsConst[targetSp]) {
                ...
                int destOff = targetSp * BLOCK_SIZE;
                long srcOffsetBytes = ctx.stackSegOffsets[targetSp] * ELEMENT_BYTES;
                MemorySegment.copy(ctx.stackSegments[targetSp], srcOffsetBytes, ctx.scratchSegment, (long) destOff * ELEMENT_BYTES, (long) n * ELEMENT_BYTES);
                ctx.stackArrays[targetSp] = ctx.scratch;
                ctx.stackOffsets[targetSp] = destOff;
                ctx.stackIsSegment[targetSp] = false;
...
// Hyperbolic Inverses
        ...
        private void doAdd(EvaluationContext ctx, int n) {
            ...
            if (lIsSeg && rIsSeg) {
                for (; k < limit; k += vl) {
                    FloatVector.fromMemorySegment(F_SPECIES, lSeg, (lSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder())
                            .add(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) + rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                }
                ...
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) + rVal;
                }
            } else if (lIsSeg) {
                for (; k < limit; k += vl) {
                    FloatVector.fromMemorySegment(F_SPECIES, lSeg, (lSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder())
                ...
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) + rArr[rOff + k];
                }
            } else if (rIsSeg && lIsConst) {
                final FloatVector laVec = FloatVector.broadcast(F_SPECIES, lVal);
                for (; k < limit; k += vl) {
                    laVec.add(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lVal + rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                }
            } else if (rIsSeg) {
                for (; k < limit; k += vl) {
                ...
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lArr[lOff + k] + rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                }
        ...
        private void doSub(EvaluationContext ctx, int n) {
            ...
            if (lIsSeg && rIsSeg) {
                for (; k < limit; k += vl) {
                    FloatVector.fromMemorySegment(F_SPECIES, lSeg, (lSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder())
                            .sub(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) - rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                ...
                final FloatVector laVec = FloatVector.broadcast(F_SPECIES, lVal);
                for (; k < limit; k += vl) {
                    laVec.sub(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lVal - rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
        ...
        private void doMul(EvaluationContext ctx, int n) {
            ...
            if (lIsSeg && rIsSeg) {
                for (; k < limit; k += vl) {
                    FloatVector.fromMemorySegment(F_SPECIES, lSeg, (lSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder())
                            .mul(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) * rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                ...
                final FloatVector laVec = FloatVector.broadcast(F_SPECIES, lVal);
                for (; k < limit; k += vl) {
                    laVec.mul(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lVal * rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
        ...
        private void doDiv(EvaluationContext ctx, int n) {
            ...
            if (lIsSeg && rIsSeg) {
                for (; k < limit; k += vl) {
                    FloatVector.fromMemorySegment(F_SPECIES, lSeg, (lSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder())
                            .div(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lSeg.getAtIndex(ValueLayout.JAVA_FLOAT, lSegOff + k) / rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
                ...
                final FloatVector laVec = FloatVector.broadcast(F_SPECIES, lVal);
                for (; k < limit; k += vl) {
                    laVec.div(FloatVector.fromMemorySegment(F_SPECIES, rSeg, (rSegOff + k) * ELEMENT_BYTES, ByteOrder.nativeOrder()))
                            .intoArray(ctx.scratch, resOffset + k);
                }
                for (; k < n; k++) {
                    ctx.scratch[resOffset + k] = lVal / rSeg.getAtIndex(ValueLayout.JAVA_FLOAT, rSegOff + k);
