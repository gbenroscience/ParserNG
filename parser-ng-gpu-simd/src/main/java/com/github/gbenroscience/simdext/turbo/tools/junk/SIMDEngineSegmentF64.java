 ...
 *
 * <p><b>Concurrency contract:</b> a single {@link SIMDVectorCompositeExpression}
 * instance may have multiple threads calling {@link SIMDVectorCompositeExpression#applyBulk}
 * concurrently (that path only touches thread-local state). However, only one
 * thread at a time may be inside {@link SIMDVectorCompositeExpression#applyBulkParallel} on a
 * given instance, since the worker pool and its {@code CoordinationContext} are
 * shared singletons for the life of the instance; concurrent parallel dispatches
 * are serialized internally to prevent state corruption, so callers should not
 * rely on overlapping parallel batches actually running concurrently.
 */
public class SIMDEngineSegmentF64 extends VectorTurboEvaluator {
    ...
    public SIMDEngineSegmentF64(MathExpression me) throws Throwable {
        super(me);
    }

    public SIMDEngineSegmentF64(MathExpression me, int numWorkers) throws Throwable {
        super(me, numWorkers);
    }

    public static final SIMDEngineSegmentF64.SIMDVectorCompositeExpression getEvaluator(MathExpression me) throws Throwable {
        return (SIMDEngineSegmentF64.SIMDVectorCompositeExpression) new SIMDEngineSegmentF64(me).compile();
    }

    public static final SIMDEngineSegmentF64.SIMDVectorCompositeExpression getEvaluator(String expr) throws Throwable {
        return (SIMDEngineSegmentF64.SIMDVectorCompositeExpression) new SIMDEngineSegmentF64(new MathExpression(expr)).compile();
    }

    public static final SIMDEngineSegmentF64.SIMDVectorCompositeExpression getEvaluator(MathExpression me, int numWorkers) throws Throwable {
        return (SIMDEngineSegmentF64.SIMDVectorCompositeExpression) new SIMDEngineSegmentF64(me, numWorkers).compile();
    }

    public static final SIMDEngineSegmentF64.SIMDVectorCompositeExpression getEvaluator(String expr, int numWorkers) throws Throwable {
        return (SIMDEngineSegmentF64.SIMDVectorCompositeExpression) new SIMDEngineSegmentF64(new MathExpression(expr), numWorkers).compile();
    }
    ...
    public final class SIMDVectorCompositeExpression extends BatchedVectorCompositeExpression implements AutoCloseable {
        ...
        public SIMDVectorCompositeExpression(int stackDepth, int blockSize) {
            ...
            // Every block-splitting computation elsewhere in this class
            // (applyBulkInternal, materialize, etc.) uses the BLOCK_SIZE
            // compile-time constant directly, NOT this constructor parameter.
            // Fail fast instead of allowing the two to silently diverge, which
            // would otherwise corrupt scratch-buffer indexing.
            if (blockSize != BLOCK_SIZE) {
                throw new IllegalArgumentException(
                        "blockSize (" + blockSize + ") must equal BatchedVectorCompositeExpression.BLOCK_SIZE (" + BLOCK_SIZE + ")");
            }

            this.masterEvalContext = ThreadLocal.withInitial(() -> new EvaluationContext(stackDepth, blockSize, varCount));

            // Reserve one participant slot for the calling (master) thread: if the
            // caller asked for N total participating threads, spawn N-1 background
            // workers so N are doing work in total once the master joins in.
            // numWorkers <= 1 means "run single-threaded" (master only, no pool) --
            // previously "numWorkers <= 2" incorrectly kept NUM_WORKERS == numWorkers
            // for 1 and 2, which spawned one MORE thread than requested (2 or 3
            // total participants instead of 1 or 2).
            if (numWorkers <= 1) {
                this.NUM_WORKERS = 0;
            } else {
            ...
            if (this.NUM_WORKERS > 0) {
                ...
                if (coreGroups == null || coreGroups.length == 0) {
                    // No usable topology info (unsupported OS/JVM combo, sandboxed
                    // environment, etc.) -- fall back to "don't pin" instead of
                    // throwing or dividing by zero below.
                    coreGroups = new int[][]{{-1}};
                }
        ...
        private void checkOpen() {
            if (isClosed) {
                throw new IllegalStateException("SIMDVectorCompositeExpression has already been closed");
            }
        }
        ...
        private static final class CoordinationContext {
            ...
            // Captures the first Throwable raised by any worker during a batch,
            // so a failure that would otherwise be silently swallowed (and leave
            // that worker's output slice un-populated) surfaces to the caller of
            // applyBulkParallel instead.
            ...
            MemorySegment[] varsSegmentArray; // one segment per variable/column (e.g. one per Arrow ValueVector)
                ...
                while (sharedCtx.running) {
                    ...
                    try {
                        if (sharedCtx.varsSegmentArray != null) {
                            // Per-column MemorySegment path (e.g. Arrow ValueVector buffers)
                        ...
                        // uncaught Error would otherwise leave signal/latch in a
                        // state that hangs the master forever in its park() wait
                        // loop below. Record the first failure and let the caller
                        // of applyBulkParallel see it, instead of silently leaving
                        // this slice of the output buffer un-populated.
                        sharedCtx.workerError.compareAndSet(null, t);
                    } finally {
        ...
        /**
         * Serializes calls to the two {@code executeParallelProcessing(...)}
         * overloads. {@code CoordinationContext} (segment refs, sample count,
         * master thread, completion latch, worker signals) is a single object
         * shared by the whole worker pool for the lifetime of this
         * {@code SIMDVectorCompositeExpression} — concurrent dispatches from
         * two calling threads would race on those fields and corrupt each
         * other's in-flight batch. Only one parallel batch may be in flight at
         * a time; single-threaded {@link #applyBulk} is unaffected since it
         * only touches thread-local state.
         */
         ...
         * only if this calling thread hasn't already been pinned. Affinity
         * is sticky — sched_setaffinity/SetThreadAffinityMask don't need to
         * be re-asserted before every dispatch — so we pin once per thread
        ...
        /**
         * Rethrows a Throwable captured from a worker (or from the master's
         * own slice) without wrapping RuntimeExceptions/Errors, and wraps
         * anything else in a RuntimeException so callers of
         * {@code applyBulkParallel} observe a real failure instead of a
         * silently under-populated output buffer.
         */
        private static void rethrow(Throwable t) {
            ...
            throw new RuntimeException("SIMD parallel worker execution failed", t);
        }
        ...
        private void executeParallelProcessing(MemorySegment varsSegment, MemorySegment outputSegment, long numSamples) {
            ...
            try {
                ...
                coordinationContext.workerError.set(null);
                ...
                // Always wait for every worker to finish, even if the master's own
                // slice threw -- workers are still writing into the shared output
                // buffer and clearPayload() must not run out from under them.
                while (coordinationContext.completionLatch.get() > 0) {
                    LockSupport.park();
                }
                coordinationContext.clearPayload();

                Throwable workerError = coordinationContext.workerError.getAndSet(null);
                if (masterError != null) {
                    if (workerError != null) {
                        masterError.addSuppressed(workerError);
                    }
                    rethrow(masterError);
        ...
        /**
         * Parallel dispatch for the per-column MemorySegment[] path (one
        ...
        // =========================================================================
        // Multi-Segment (per-column) MemorySegment API — for zero-copy
        ...
        private static final class EvaluationContext {
            ...
            // True zero-copy MemorySegment-backed stack support — one segment per
            ...
            /**
             * Initializes this context for a block whose variables are
        ...
        /**
         * Per-column MemorySegment[] variant of the bulk internal loop.
        ...
        private void materialize(EvaluationContext ctx, int targetSp, int n) {
            if (ctx.stackIsConst[targetSp]) {
                ...
                // On-demand copy of off-heap segment data into scratch — only paid
                ...
                // all; see doAddSeg/doSubSeg/doMulSeg/doDivSeg's segment-native fast paths below.
                int destOff = targetSp * BLOCK_SIZE;
                ...
                switch (opcode) {
                    ...
                    case OP_LOAD -> {
                        ...
                        if (ctx.segVariables != null) {
                            // True zero-copy read — point the stack slot directly at the
                    ...
                    // The segment-aware 8-branch implementations (doAddSeg/doSubSeg/
                    // doMulSeg/doDivSeg) are only needed on the zero-copy
                    // MemorySegment[] call path (ctx.segVariables != null). Every
                    // other call path (flat array, double[][], or a single
                    // concatenated MemorySegment staged into segmentBlockVars) never
                    // produces a segment-backed stack slot, so it is routed to the
                    // lean array-only 4-branch implementations (doAddArr/doSubArr/
                    // doMulArr/doDivArr) instead, avoiding the extra branch/field
                    // overhead of the segment-aware path on the common hot path.
                    case OP_ADD -> {
                            doAddSeg(ctx, n);
                    }
                    case OP_SUB -> {
...
// Reciprocal Trig (SEC, CSC, COT) & Variants
                    ...
                    case OP_COT_GRAD -> {
                        materialize(ctx, ctx.sp - 1, n);
                        // FIX: was `SIMDCommandEvaluator.VectorMath.cotGrad(...)`, an
                        // undefined/unimported class reference — a compile error. The
                        // implementation lives on this class's own VectorMath.
...
// Hyperbolic Inverses
        ...
        /**
         * Segment-aware ADD — the full 8-branch implementation covering
         * every combination of segment-backed, array-backed, and constant
         * operands. Used only when {@code ctx.segVariables != null} (the
         * zero-copy {@code MemorySegment[]} call path). See {@link #doAddArr}
         * for the lean counterpart used by every other call path.
         */
        private void doAddSeg(EvaluationContext ctx, int n) {
        ...
        /**
         * Segment-aware SUB — the full 8-branch implementation. See
         * {@link #doAddSeg} for the rationale.
         */
        private void doSubSeg(EvaluationContext ctx, int n) {
            final int rSp = --ctx.sp;
            final boolean rIsConst = ctx.stackIsConst[rSp];
            final boolean rIsSeg = ctx.stackIsSegment[rSp];
            final double[] rArr = ctx.stackArrays[rSp];
        ...
        /**
         * Segment-aware MUL — the full 8-branch implementation. See
         * {@link #doAddSeg} for the rationale.
         */
        private void doMulSeg(EvaluationContext ctx, int n) {
            final int rSp = --ctx.sp;
            final boolean rIsConst = ctx.stackIsConst[rSp];
            final boolean rIsSeg = ctx.stackIsSegment[rSp];
            final double[] rArr = ctx.stackArrays[rSp];
        ...
        /**
         * Segment-aware DIV — the full 8-branch implementation. See
         * {@link #doAddSeg} for the rationale.
         */
        private void doDivSeg(EvaluationContext ctx, int n) {
            final int rSp = --ctx.sp;
            final boolean rIsConst = ctx.stackIsConst[rSp];
            final boolean rIsSeg = ctx.stackIsSegment[rSp];
            final double[] rArr = ctx.stackArrays[rSp];
