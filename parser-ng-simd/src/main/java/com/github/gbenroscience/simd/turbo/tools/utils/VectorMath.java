...
// Based on your switch case, unary GEGLU passes 'x' through SIMD but runs geglu() on the tail.
    ...
    private static final double SAFE_INTEGRAL_BOUND = 4503599627370496.0; // 2^52

    public static void ceil(int base, int n, double[] s) {
        int limit = SPECIES.loopBound(n);
        int k = 0;

        for (; k < limit; k += SPECIES.length()) {
            DoubleVector v = DoubleVector.fromArray(SPECIES, s, base + k);

            // False for |v| >= 2^52 (already integral), and also false for
            // +-Infinity and NaN -- one guard covers all three "leave as-is" cases.
            var inSafeRange = v.abs().compare(VectorOperators.LT, SAFE_INTEGRAL_BOUND);
            // Zero (either sign) is also already its own ceiling/floor, and
            // routing it through here preserves -0.0's sign, which the D2L/L2D
            // round trip would otherwise silently flip to +0.0 (see Bug 3).
            var isZero = v.compare(VectorOperators.EQ, 0.0);
            var passthrough = inSafeRange.not().or(isZero);

            DoubleVector truncated = (DoubleVector) v.convert(VectorOperators.D2L, 0)
                    .convert(VectorOperators.L2D, 0);

            var truncatedLow = truncated.compare(VectorOperators.LT, v);
            DoubleVector ceiled = truncated.blend(truncated.add(1.0), truncatedLow);

            DoubleVector result = ceiled.blend(v, passthrough);
            result.intoArray(s, base + k);
        }

        for (; k < n; k++) {
            s[base + k] = Math.ceil(s[base + k]);
        }
    }

    public static void floor(int base, int n, double[] s) {
        int limit = SPECIES.loopBound(n);
        int k = 0;
        ...
        for (; k < limit; k += SPECIES.length()) {
            ...
            DoubleVector floored = truncated.blend(truncated.sub(1.0), truncatedHigh);

            DoubleVector result = floored.blend(v, passthrough);
            result.intoArray(s, base + k);
        }

        for (; k < n; k++) {
            s[base + k] = Math.floor(s[base + k]);
        }
    }

    public static void round(int base, int n, double[] s) {
        int limit = SPECIES.loopBound(n);
        int k = 0;
        ...
        for (; k < limit; k += SPECIES.length()) {
            ...
            // half-integers, where round-half-up and round-half-away-from-zero
            // disagree. No sign branch needed: floor already handles both signs.
            DoubleVector shifted = v.add(0.5);

            DoubleVector truncated = (DoubleVector) shifted.convert(VectorOperators.D2L, 0)
                    .convert(VectorOperators.L2D, 0);

            // Truncation rounds toward zero; turn it into floor() by subtracting
            // 1 wherever it overshot (shifted was negative with a fractional part).
            var overshot = truncated.compare(VectorOperators.GT, shifted);
            DoubleVector rounded = truncated.blend(truncated.sub(1.0), overshot);
