...
// Thread-local cache allocation helper for flat translation buffers
    ...
    public class BatchedVectorCompositeExpression implements SIMDCompositeExpression {
        ...
        protected final ThreadLocal<double[]> FLAT_SCRATCH_STACK = ThreadLocal.withInitial(()
                -> new double[VectorTurboEvaluator.this.stackDepth * BLOCK_SIZE]
        );
         ...
         * locality; {@code false} for flat, non-tiledExecution bulk processing
         *
        ...
        private void applyBulkInternal(double[][] variables, int dataSize, double[] output, int startIdx, int length) {
            ...
            // Thread-local scratch space acquisition & sizing guard
            double[] scratch = FLAT_SCRATCH_STACK.get();
            if (scratch == null || scratch.length < (VectorTurboEvaluator.this.stackDepth * BLOCK_SIZE)) {
                scratch = new double[VectorTurboEvaluator.this.stackDepth * BLOCK_SIZE];
                FLAT_SCRATCH_STACK.set(scratch);
            }
         ...
         * constants.
         */
        public static final int chooseOptimalBlockSize(int dataSize) {
        ...
        public void applyBulkParallel(double[] flatVariables, double[] output) {
            ...
            dispatchToWorkerRing(flatVariables, output, length);
        }
