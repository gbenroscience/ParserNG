...
//if so,the user cannot define it by var x = 2; instead use x =9; to change the value in x.
    ...
    /**
     * Parses commands used to insert and update Variables
     * loaded into the VARIABLES attribute of objects of this
     * class.
     */
    ...
    /**
     *
     * @return the ArrayList object that stores the
     * Variable data of objects of this class.
     */
     ...
     * {@link VariableManager#VARIABLES} {@link Map}.
     * @param variables A {@link Map} of {@link Variable} objects.
     * @param clearFirst If true, then the {@link VariableManager#VARIABLES} is cleared first before
     *                   new content is loaded into it.
     */
    ...
    /**
     *
     * @param variableName The name attribute of the variable
     * we are searching the variable store for.
    ...
    /**
     *
     * Parses a command that creates
     * a single variable or changes its value.
...
return null;
     ...
     *
     * Parses a command that creates
     * or changes the value of variables.
     * @param cmd The command string to parse.
     */
    ...
    /**
     * Saves stored vasriables and updates the UI
     * that renders the variables.
     */
     ...
     * store and loads them from persistent storage
     */
     ...
     * @return the Variable object that has the name supplied
     * if it exists.
     * If no such Variable object exists, then it returns null.
     */
     ...
     * object from a VariableManager based on its name.
     * @param vName The name of the Variable object.
     * @return the Variable object that has that name or
 ...
 */
     ...
     * @param var the name of the Variable object to be added to the Variable Registry
     */
    public static void add(Variable var) {
         ...
         * @param vars the variable args list of the Variable objects to be added
     ...
     * @return a command string that initializes the variable names to 0.0
     * e.g if the array is [a,v,b,m,n], then the output is:
...
synchronized (VARIABLES) {
     ...
     * parse a variable initialization
     * or modifying command string.
     * For example var x = 2;var y=3; var z =4;
     * x=9;y=3.123;const k = 3x+z; k=34;
     * var p,q,r=32; const r,s,t=25;
     * z=3.123;
     * All that is needed to execute the parse
     * is to create an object of the class
     * with the available constructor.
     * At the end of the object creation,
     * the parse has been executed and if
     * it is successful, a call to
     * getAccumulator() will return a java.util.List
     * object filled with the Variable objects embedded in the command.
     * Also at the end of the object creation( the call to the constructor of this class)
     * The VARIABLES attribute of the containing class will have been fed with the Variable objects
     * embedded in the command parsed here. The feeding could be the insertion of the Variable objects,
     * if the store doe not already contain them,or updating Variable objects in the store with the incoming ones
     * if they have the same name.
     * The behavior of the parser is such that it interpretes and executes the code on the fly.
     * This means that it will stop inserting data in the VARIABLES only when it detects
     * error in the code.
     *
     *
     */
    public class CommandInterpreter{
        ...
        /**
         * At every step of parsing,
         * this boolean records if the
        ...
        /**
         *
         * @param command The variable creation command
         * string.
         */
        ...
        /**
         * Takes a String object that represents a variable name,
         * checks if the variable exists in the store and then returns the
        ...
        /**
         * A line is a statement that ends with a semicolon that is...';'
         * This method generates lines of instructions from the
         * command that is input.
         * @return an ArrayList object containing all lines of instructions in the
         ...
         * @return true if a constant< A Variable object that is constant> by that name is found in the accumulator
         * of variables during expression parsing or in the variable store.
         * It returns false otherwise.
         * A return value of false means one of two things:
         * 1. The item was found and was found not to be a constant but a variable.
         * 2. The item was not found at all.
         */
...
return v==null?false:v.isConstant();
         ...
         *
         * @param name The name of the Variable whose type is to be checked.
         * @return true if a variable< A Variable object that is not constant> by that name is found in the accumulator
         * of variables during expression parsing or in the variable store.
         * It returns false otherwise.
         * A return value of false means one of two things:
         * 1. The item was found and was found not to be a variable but a constant.
         * 2. The item was not found at all.
         */
        ...
        /**
         * Validates a single line of
         * Variable initialization or updating
         * statement.
         * @param line The line.
...
//= is found in the string.
         ...
         * name = value.e.g t=4
         * Action:
         * Check the variables store to see if a variable
         * by this name exists. If so,the line is correct.
         * But make sure that no line before this one declares it again.
         * e.g store has variable,then var t =4; t=2;.....the t=2; is the scenario now.
         * if the store has declared it then we don't need to declare it before using it
         * again.
         * Else,check if this variable has been declared in a line before this
         * one. If so the line is correct.Else it is wrong,terminate the whole process.
         *
         * Scenario:
         * var|const name = value|expression. var t = 4 or const w=2
         * Action:
         * Check the variables store to see if a variable
         * by this name exists. If it does exist,the line is wrong,terminate the whole process.
         * Else it is correct.
         *
         * Scenario:
         * name1,name2,name3....,nameN = value|expression e.g. var r,a,t,d=32 or const g,h,i,j=3.14
         * Action:
         * Check the variables store to see if any of these variables have names
         * that match that of any variable in the store.
         * If they do, the code is wrong.
         * Else it is correct if and only if...their exists no line before
         * this one that declares any of the variables in this line.
         * @return true if the line is valid
         */
...
//multiple assignment. e.g  a,v,c,d,r=value|expression...is line format.
         ...
         * @return true if the store of variables
         * of objects of the containing class contain
         * a variable or constant having the parameter name.
         */
