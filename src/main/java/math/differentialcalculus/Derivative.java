...
public class Derivative {
     ...
     * e.g. diff(sin(3*x+1))
     * @param expression  The expression to be parsed into <code>mathExpr</code> and <code>variable</code>.
     *
     *
     */
    Derivative(String expression) throws Exception{
        if(isAutoGenNameFormat(expression)){
            this.mathExpr = builder.getManager().lookUp(expression).getExpression();
        }
        else{
            this.mathExpr = expression;
        }
    ...
    /**
     *
     * @return an ArrayList containing this object's data in terms
     ...
     *
     * If the diff(@(x)sin(x),2) form is specified, then the function is differentiated 2 times
     * and the result is returned.
     *
     * If the diff(@(x)sin(x),2,3) form is specified, then the function is differentiated 3 times
