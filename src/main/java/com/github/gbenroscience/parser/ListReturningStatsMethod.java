...
this one.
    ...
    public static void setErrorMessage(String errorMessage) {
        ListReturningStatsMethod.errorMessage = errorMessage;
    }
                ...
                loopBackwards:
                {
                    ...
                    for (int i = index - 1; i >= 0; i--) {
                        ...
                        if (isOpeningBracket(scan.get(i)) && Method.isUnaryPreOperatorORDefinedMethod(scan.get(i - 1))) {
                            ...
                            if (compIndex > closeBracket.getIndex()) {
                                valid = false;
                                errorMessage += "\n Bad Syntax! Do Not Embed " + name + " In Parentheses Belonging To " + scan.get(i - 1);
                                break loopBackwards;
                            }
                        }//end if
                        ...
                        if (isClosingBracket(scan.get(i)) || Method.isStatsMethod(scan.get(i))) {
                            errorMessage += "\n Ending Backwards Validation For " + name;
                            break loopBackwards;
                        }//end if
