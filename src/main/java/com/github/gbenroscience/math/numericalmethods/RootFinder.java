 ...
 * equations. They combine a variety of algorithms in an
 * if-fail-switch-algorithm fashion to iteratively deduce the roots.
 * <ol>
 * <li>
 * <b>The Secant Algorithm.</b>
 * <p>
 * The first algorithm is the secant algorithm. It requires 2 values of x
 * between which it seeks out the root. If it does not find it in the specified
 * range however, it may search outside the range.
 *
 * If the secant fails, the object automatically switches over to another of its
 * methods...
 * </p>
 * </li>
 * <li>
 ...
 * <p>
 * This algorithm searches for a root ONLY within the specified range and
 * returns one if it exists. However if it fails, the object again automatically
 * switches over to an highly unpredictable algorithm called here:
 * </p>
 * </li>
 * <li>
 * <b> The Self-Evaluating Algorithm. </b>
 * <p>
 * Two variants of these are used here and due to its unstable nature,it is the
 * algorithm of last resort. It searches for a root starting at the first limit
 * specified for x but the direction of search is not guaranteed due to its
 * instability.
 * </p>
 * </li>
 *
 * </ol>
 * If both flavors of this algorithm fail, then an error report is generated.
 *
 * The search for the root is intensive and if no root is found, it is usually
 * because no real root exists for the function in the specified range.
 *
 * <br><br>
 * <b>Usage:</b><br>
 ...
 * </b><br>
 * This will try to seek out the zeroes of 2x^3-5x+sin(x)-1 between x = -3 and x
 * = 5 depending on the algorithm in use.
 *
 * If however, the variable has not been initialized before in the workspace or
 * has been initialized but is not visible to the currently evaluating object of
 * this class then an example could be:<br>
 * <b>
 ...
 * <b>
 * <strong color="red">
 * CAUTION!!!!! Always end your equations with "=0"<br>
...
public class RootFinder {
    ...
    /**
     *
     * @param expression An input expression containing information about the
     * function whose roots are needed. e.g. var x=0;//initialization
     * root(@(x)3sin(x)-4x+1,2) root(@(x)3sin(x)-4x+1,2,8)
     * root(@(x)3sin(x)-4x+1,2,8,2000), x1, x2 and iterations
     *
     */
    ...
    /**
     *
     * @param function A String to be used to initialize the Function attribute
     * of this class. It could be anonymous e.g @(x)sin(x+1) or be the name of a
     * pre-defined function.
     * @param x1 A starting value for iteration...will automatically be set to
     * the smaller value of the boundaries.
     *
     *
     */
    public RootFinder(String function, double x1) {
        this(new Function(function), x1, x1 + 2);
    }//end constructor
    ...
    public RootFinder(Function function, double x1, double x2) {
        this(function, x1, x2, 2000);
    }//end constructor

    /**
     *
     * @param function A Function to be used to initialize the Function
     * attribute of this class.
     * @param x1 A starting value for iteration...will automatically be set to
     * the smaller value of the boundaries.
     * @param x2 A bounding value for the iteration. The roots are not bound to
     * lie between x1 and x2.
     * @param iterations The number of times a particular root finding method of
     * this class will run before it switches to another method
     *
     */
     ...
     * <br>
     * <b color='red'>
     * Actually, the lower-limit and upper-limit values specified does not
     * guarantee that the root returned will be between the values specified. In
     * fact, the root could be far away from the values specified.
     * <br><br><br><br>
     * </b>
     *
     *
     ...
     *
     * @(x)sin(3x-5),2,5.//assuming default number of iterations which will be
     * computed automatically
     * @(x)sin(3x-5),2,5,50.//specifies 50 iterations.
     *
     * Using defined functions: F,2,5.//assuming default number of iterations
     * which will be computed automatically F,2,5,50.//specifies 50 iterations.
     *
     * Please ensure that the function is continuous in the specified range.
     *
     * @param expression The expression containing the function to integrate and
     * the lower and upper boundaries of integration.
     *
     * @return an array containing:: At index 0.....the expression to expand. At
     * index 1.....the lower limit of expansion. At index 2.....the upper limit
     * of expansion. At index 3(optional)...the degree of the polynomial up to
...
//The expression should look like...function,x1,x2,iterations(optional)
            ...
            /**
             * test for a third argument and report an error if one is found,
     ...
     * root(@sin(x+1),4,7) root(F,4,7) where F is a function that has been
     * defined before in the workspace.. and so on.
     *
     * Simplifies the list to the form root(funcName,x1,x2) or
     * root(funcName,x1,x2,iterations)
     *
     */
    public static void extractFunctionStringFromExpression(List<String> list) {
        ...
        if (methodName.equals("root") && list.get(1).equals("(") && list.get(list.size() - 1).equals(")")) {
            ...
            int sz = list.size();
            boolean noArgs = sz == 4;
            args1 = sz >= 6 ? list.get(4) : null;//compulsory---xLower
            args2 = sz >= 8 ? list.get(6) : null;//optional---xUpper
            args3 = sz >= 10 ? list.get(8) : null;//optional---iterations

            if (noArgs) {//no number args, only the function whose root is desired was supplied
                return;
            }
                    ...
                    if (Number.isNumber(args2)) {
                        if (Operator.isClosingBracket(list.get(7))) {//two number args
                            ...
                            if (Number.isNumber(args3)) {//three number args
                                ...
                                System.err.println("For root(" + args1 + "," + args2 + "," + args3 + ",... " + args3 + " MUST be a number");
                                list.clear();
                            }
                        }
                    } else {
                        System.err.println("For root(" + args1 + "," + args2 + ",... " + args2 + " MUST be a number");
                        list.clear();
                    }
                }
            } else {
                System.err.println("Only a number is allowed after the function: " + functionName);
                list.clear();
                //invalid
            }
        }
    ...
    /**
     * This method starts with the secant algorithm and if this fails, switches
     * to the bisection algorithm, which if it does not succeed switches to a
     * form of the self-evaluating algorithm. If this final attempt does not
     * return a result after about {@link RootFinder#iterations} counts, it
     * switches to another form of the same algorithm which will run for another
     * {@link RootFinder#iterations} counts and if it fails, will deliver an
     * error report or switch to another iterative method when this becomes
     * available.
     *
    ...
    private class Bisection {
         ...
         * @return the root of the equation.
         */
        private double findRoot() throws Exception {
...
//(abs(df_x) >= 0.0 && abs(f_x)!=abs(df_x) && count < iterations)
//abs( x2 - x1 ) >= 5.0E-16
            while (abs(f2) >= 0.0 && abs(f1) != abs(f2) && count < iterations) {
...
//if rootfinding by bisection fails, switch to an alternative method.
    ...
    private class Newtonian {
        ...
        public double findRoot() throws Exception {
            ...
            double ratio = 0;
            //iterate
            while ((!approxEqualsZero(abs(ratio = f_x / df_x)) && count < iterations)) {
...
// did loop terminate because count >= iterations? if so then the root finder failed again....
            //so switch to something else. Place an alternative method in this loop.
            if (count >= iterations || iterationfailed) {
    ...
    private class Secant {
        ...
        public double findRoot() throws Exception {
            ...
            boolean iterationfailed = false;
            //iterate
            while ((!approxEqualsZero(abs(f2)) && !approxEqualsZero(abs(f1)) && count < iterations)) {
...
// did loop terminate because count >= iterations? if so then the root finder failed again....
    ...
    private class SelfEvaluator {
        ...
        public double findRoot() throws Exception {
            ...
            int count = 0;
            while (abs(f1) >= 5.0E-16 && count < iterations) {
            ...
            // did loop terminate because count >= iterations? if so then the root finder failed
            //so switch to something else. Place an alternative method in this loop.
            if (count >= iterations && abs(f1) >= 5.0E-16) {
                count = 0;
                x1 = x1backup;
     ...
     *
     * @param iterations The number of iterations before switching to another
     * method
     */
    ...
    /**
     *
     * @return The number of iterations before switching to another method
     */
    ...
    public static void main(String args[]) {
        ...
        RootFinder finder = new RootFinder("root(f,0.2, 10)");
