...
public class NumericalIntegral {
    ...
    /**
     * Use this to integrate without using the integral symbol. Here, the intg()
     * command containing the function and the bounds are specified.
     */
     ...
     * @param function The name of a Function that has been defined before in
     * the WorkSpace or an anonymous Function just specified.
     */
     ...
     * whose integral is to be evaluated and the limits of integration. For
     * example: function,2,4....means integrate function between horizontal
     * coordinates 2 and 3. Direct examples would be: sin(x+1),3,4
...
//no info about the number of interations specified,so set default number of iterations.
     ...
     *
     * Simpson's rule requires an even number of rectangular strips and so an
     * odd number of ordinates(y-coordinates) The x distances must also be
     * equal. h= (xUpper-xLower)/(2*m)
     */
    public String findSimpsonIntegral() {
        ...
        fun.updateArgs(xLower);
        double first = fun.solveGeneric().scalar;

        fun.updateArgs(xUpper);
        double last = fun.solveGeneric().scalar;
        ...
        for (; x < (xUpper - h);) {
            ...
            try {
                if ((count % 2) == 0) {
                    fun.updateArgs(x);
                    sumEven += fun.solveGeneric().scalar;
                }//end if
                else if ((count % 2) == 1) {
                    fun.updateArgs(x);
                    sumOdd += fun.solveGeneric().scalar;
                }//end else if
                ++count;
            }//end try
     ...
     * @param h
     */
    public double findSimpsonIntegral(double h) {
        ...
        if (xUpper == this.xUpper) {
            ...
            fun.updateArgs(xLower);
            first = fun.solveGeneric().scalar;

            fun.updateArgs(xUpper);
            last = fun.solveGeneric().scalar;
            count = 1.0;
     ...
     */
    public double findTrapezoidalIntegral() {
     ...
     * @return the integral of the function using the trapezoidal rule.
     */
    public double findTrapezoidalIntegral(double h) {
        ...
        for (; x < (xUpper);) {
            try {
                fun.updateArgs(x);
                mainSum += fun.solveGeneric().scalar;
                ++count;
                x = xLower + count * h;
            }//end try
            catch (NumberFormatException numErr) {
...
//≤≤≤≥
            ...
            if (Math.abs(xUpper - xLower) < dx) {
                ...
                if (xLower <= xUpper) {
                    ...
                    if (x < xUpper) {
                         ...
                         * x and xUpper are so close and in the case of the
                         * polynomial integral, computing it uses matrices which
                         * means that row-reduction will fail if the
                         * coefficients of the matrices are too close due to the
                         * computational values of x and y..which are close. If
                         * such an exception occurs we can safely neglect it
                         * since it means that the area we are considering is
     ...
     *
     * Simplifies the list to the form intg(funcName,x1,x2) or
     * intg(funcName,x1,x2,iterations)
     *
     */
    ...
    class Parser {
        ...
        /**
         * Method that processes the format that this software will recognize
         * for user input of an integral expression.
         *
         ...
         * computed automatically sin(3x-5),2,5,50.//specifies 50 iterations.
         * Please ensure that the function is continuous in the specified range.
         *
         * @param expression The expression containing the function to integrate
         * and the lower and upper boundaries of integration.
         *
         * @return an array containing:: At index 0.....the expression to
         * integrate At index 1.....the lower limit of integration At index
         * 2.....the upper limit of integration. At index 3(optional)...the
         * number of iterations to employ in evaluating this expression.
         *
         * intg( F,0,2,3 ) intg(@3x+1,0,2,5) OR intg(F(x),0,2,5) OR
         * intg(F,0,2,5)
         *
         */
...
//The expression should look like...function,x1,x2,iterations(optional)
                ...
                /**
                 * test for a fourth argument and report an error if one is
                 * found, else exit test quietly.
                 */
                lastCommaIndex = expression.lastIndexOf(",");
                try {
                    args[0] = Double.parseDouble(expression.substring(lastCommaIndex + 1).trim());
...
//setIterations(5);
         ...
         * 2.....the upper limit of integration. At index 3(optional)...the
         * number of iterations to employ in evaluating this expression.
         * F(x)=3x+1; ∫( F,0,2,3 )dx ∫(@3x+1,0,2,5)dx OR ∫(F(x),0,2,5)dx
         *
         */
