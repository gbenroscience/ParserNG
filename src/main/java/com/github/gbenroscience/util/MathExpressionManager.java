 ...
 * functions database (actually an ArrayList of MathExpression objects) then it
 * just retrieves the scanned and validated code and solves it. Else,it must go
 * through all the steps and then solve it. This happens when: 1. The expression
 * is a new one. 2. The expression has been computed before but to limit memory
 * usage, it has been deleted from memory.
 *
 * @author GBEMIRO
 */
public class MathExpressionManager {
    ...
    /**
     * Whether to compute trigonometric functions in degrees, rads or grads. Set
     * to 0 for degrees, set to 1 for rads and set to 2 for grads.
     */
    ...
    /**
     * stores a MathExpression in objects of this class. The storage is done in
     * such a way that the object is inserted from the front.
     *
    ...
    /**
     *
     * @return true if this object has no MathExpression objects to manage.
     */
     ...
     * Else if the expression cannot be optimized, it has to interpret and then
     * evaluate it. It then stores the expression.
     *
        ...
        try {
            ...
            if (mathExpr != null && !mathExpr.isEmpty() && exprCount == 1) {
                if ((f = getFunctionByExpression(mathExpr)) != null) {//optimize function
                    ...
                    /**
                     * can take care of the raw input since it will still scan
                     * for variables and constants in the constructor
                     */
