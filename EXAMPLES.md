...
ParserNG now allows you to write a math expression and run it straight on your GPU!
...
It **beats exp4J, and com.expression.parser on evaluation speed** across every kind of expression — from simple algebra to heavy trig, matrices, and calculus; and manages to beat Janino, the gold standard on many expressions, while rivalling it on a host of other expressions  
...
![Latest Version](https://img.shields.io/badge/version-3.0.8-success)

> **3.0.8** introduces **Turbo Scalar** and **Turbo Matrix** compiled paths + massive speed improvements via strength reduction, constant folding, and O(1) frame-based argument passing.
...
- Introduction of if(condition, trueValOrExpr, falseValOrExpr), &&(AND) and ||(OR) operators
- **Turbo Mode** — compile once, evaluate millions of times per second (Scalar + Matrix paths)
- Point swarm rotation
- Nth-order Automatic Differentiation, Symbolic differentiation (`diff`) + resilient numerical integration (`intg`) that handles difficult expressions
...
https://central.sonatype.com/artifact/com.github.gbenroscience/parser-ng/3.0.8
...
```bash
java -jar parser-ng-3.0.8.jar "sin(x) + cos(x)"
java -jar parser-ng-3.0.8.jar "eigvalues(R=@(5,5)(...))"
java -jar parser-ng-3.0.8.jar help
java -jar parser-ng-3.0.8.jar -i          # interactive mode
```
...
- [LATEST.md](LATEST.md) — what’s new in 3.0.8
- Javadoc: https://javadoc.io/doc/com.github.gbenroscience/parser-ng/3.0.8
...
**ParserNG 3.0.8** — faster than the competition, stronger on matrices, and now with real Turbo Scalar + Turbo Matrix compiled power.
