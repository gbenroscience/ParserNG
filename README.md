...
| **3.0.0** | `SIMDEngineEvaluator` and `SIMDCommandTurboEvaluator` add CPU pinning (best on Linux): 2 workers on 2 cores ≈ 1.8×–2.0× the work of 1 worker on 1 core. `SIMDEngineEvaluator` edges out `SIMDCommandTurboEvaluator` by a few ns/op. Both live in **`parser-ng-gpu-simd`** (JDK 22+) — the module that also houses the star of this release: native **GPU bulk evaluators for CUDA and OpenCL**. |
...
* **Release Artifact Logs:** [LATEST.md](LATEST.md) — Change logs and technical notes for v3.0.0.
