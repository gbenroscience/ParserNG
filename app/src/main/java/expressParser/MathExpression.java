...
/**
 *
 * This class models a mathematical parser. It is designed to handle diverse
...
//if DRG==1, it is done in radians and if it is 2, it is done in grads.
    ...
    public void unBracketDataSetReturningStatsOperators() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            if (isListReturningStatsMethod(scanner.get(i))) {
                int I = i - 1;
                /**
                ...
                while (isOpeningBracket(scanner.get(I))) {
                    ...
                    if (I > 0 && !isListReturningStatsMethod(scanner.get(I - 1))) {
    ...
    public void statsVerifier() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            if (isListReturningStatsMethod(scanner.get(i)) && isOpeningBracket(scanner.get(i + 1))) {
                ...
                for (int i = 0; i < scanner.size(); i++) {
                    try {
                        if (isListReturningStatsMethod(scanner.get(i)) && isOpeningBracket(scanner.get(i + 1))) {
                            ...
                            if (isBracket(scanner.get(i - 1)) && !isNumberReturningStatsMethod(scanner.get(i - 2))
                                    && !isListReturningStatsMethod(scanner.get(i - 2))) {
    ...
    public void codeModifier() {
        ...
        if (correctFunction) {
            ...
            for (int i = 0; i < scanner.size(); i++) {
                 ...
                 */
                try {
                    if ((isNumber(scanner.get(i)) || (isVariableString(scanner.get(i)) && !isDefinedMethod(scanner.get(i)))) && (isUnaryPreOperatorORDefinedMethod(scanner.get(i + 1))
    ...
    public void detectKeyOperators() {
        for (int i = 0; i < scanner.size(); i++) {
            ...
            } else if (isNumberReturningStatsMethod(scanner.get(i))) {
...
//check for the various valid arrangements for all members of the function.
                ...
                //Variables
                if (isVariableString(scanner.get(i)) && !isUserDefinedFunction(token)) {
                    try {
                        ...
                        if (!isBracket(scanner.get(i + 1)) && !isBinaryOperator(scanner.get(i + 1))
                                ...
                                && !isUnaryPreOperator(scanner.get(i + 1)) && !isNumberReturningStatsMethod(scanner.get(i + 1))
                                && !isLogToAnyBase(scanner.get(i + 1)) && !isAntiLogToAnyBase(scanner.get(i + 1)) && !isNumber(scanner.get(i + 1))
                                && !isVariableString(scanner.get(i + 1))) {
                            //processLogger.writeLog("Mathron Does Not Allow "+expression+" To Combine The MathExpression Members \""+scanner.get(i)+"\" And \""+scanner.get(i+1)+"\"PLUS As You Have Done.\n");
            ...
            while (brac.length > 0) {
                ...
                if (!correctFunction) {
                    ...
                    try {
                        ...
                        try {
                            isMethod = isMethodName(myScan.get(indexOpenInMyScan - 1));
                        }//end try
                        catch (IndexOutOfBoundsException indexErr) {
                            isMethod = false;
                        }
                        ...
                        if (!isMethod) {
                            ...
                            try {
                                ...
                                if (isStatsMethod(executable.get(0))) {

                                    if (!isUserDefinedFunction(executable.get(0))) {
                                        Method.run(executable, DRG);
                                    }//end if
                                    else if (isUserDefinedFunction(executable.get(0))) {
...
//[3,4,2,sin,2,sin,(,3,),3,*,cos,5,(,3,+,(,6,-,78,),)]
        ...
        while (scanner.size() > 1) {
            ...
            try {
                ...
                j = prevIndexOf(scanner, i, "("); //index of enclosing bracket of ) above
