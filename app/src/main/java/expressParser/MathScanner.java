...
import math.numericalmethods.NumericalDerivative;
...
import expressParser.methods.Method;
...
//
    ...
    public void splitStringOnMethods_Variables_And_Operators() {
        CustomScanner cs = new CustomScanner(scannerInput, true, VariableManager.VARIABLES.keySet().toArray(new String[]{}),getAllFunctions(), operators);
        ...
        for (int i = 0; i < scanner.size(); i++) {
             ...
             */
                    ...
                    !isListReturningStatsMethod(scanner.get(i + 1))  ) {
                /**
                 * Check if the bracket owner allows for multiplication.
                 */
                int open = Bracket.getComplementIndex(false, i, scanner);
                if(open>0 && !isAtOperator(scanner.get(open-1)) && !isListReturningStatsMethod(scanner.get(open-1))){
                    scanner.add(i + 1, "*");
                }
            }
        ...
        for (int i = 0; i < scanner.size(); i++) {
            try {
                if (isMethodName(scanner.get(i)) && (isNumber(scanner.get(i + 1)) || isMethodName(scanner.get(i + 1)))) {
...
// intg,(,@,(x),log(x,2),4,2,5)
            ...
            else if (isMatrixMethod(token) && nextToken.equals("(")) {
    ...
    public void validateInputAfterSplitOnMethodsAndOps() {
        for (int i = 0; i < scanner.size(); i++) {
            try {
                 ...
                 */
                if ((isFunctionOperatingMethod(scanner.get(i)) || isStatsMethod(scanner.get(i))) && isOpeningBracket(scanner.get(i + 1))) {
                 ...
                 */
                else if ((isNumber(scanner.get(i)) || (isVariableString(scanner.get(i)) && !isDefinedMethod(scanner.get(i))))
                 ...
                 */
                else if (isNumber(scanner.get(i)) && isDefinedMethod(scanner.get(i + 1))) {
                    scanner.add(i + 1, "*");
                    i++;
                }//end if
                /**
        ...
        for (int i = 0; i < scanner.size(); i++) {
            try {
                ...
                if (!isOperatorString(scanner.get(i)) && !isVariableString(scanner.get(i))
                        && !validNumber(scanner.get(i)) && !isMethodName(scanner.get(i))) {
...
//enable interpretation of things like 3^-4 or 3^+4 i.e ^- or ^+ patterns
        ...
        for (int i = 0; i < scanner.size(); i++) {
             ...
             */
            if (i+1<sz && isFunctionOperatingMethod(scanner.get(i)) && isOpeningBracket(scanner.get(i + 1))) {
                int close = Bracket.getComplementIndex(true, i + 1, scanner);
                i = close;
                continue;
            }//end if
...
//enable interpretation of things like 3^-4 or 3^+4 i.e ^- or ^+ patterns
                ...
                if (isClosingBracket(scanner.get(i + 1))) {//you have a match for an unnecessary wasted bracket
                    ...
                    if (i - open == 2) {//confirm that the bracket has only 1 token inside it
                        if(open >  0){//open bracket is beyond 0
                            ...
                            if (!isVariableString(token) && !isAtOperator(token) && !isMethodName(token) && !isUnaryPreOperator(token) ) {
                                scanner.remove(i);
                                scanner.remove(open);
                                i -= 2;
                                continue;
                ...
                if (i + 1 < scanner.size() ) {
                    ...
                    if (isClosingBracket(scanner.get(i + 1))) {//you have a match for an unnecessary wasted bracket
                        ...
                        if (inner_open != -1 && outer_open != -1 && inner_open - 1 == outer_open) {
                            ...
                            if(outer_open>1 && isLogToAnyBase(scanner.get(outer_open-1))){
                                continue;
                            }
                            scanner.remove(i);
                            scanner.remove(inner_open);
    ...
    public static void extractFunctionStringFromExpressionForMatrixMethods(List<String> list){
        ...
        if(list.indexOf("(") == list.lastIndexOf("(") && list.indexOf(")") == list.lastIndexOf(")")){
            ...
            if(sz == 4 || sz == 5 ){
                if(isMatrixMethod(list.get(0)) && isOpeningBracket(list.get(1)) && isUserDefinedFunction(list.get(2))  ){
                    if(sz==4 && isClosingBracket(list.get(3))){
                        Method.run(list, 1);
                    }
                    else if(sz==5 && (isUserDefinedFunction(list.get(3))||isNumber(list.get(3))||isVariableString(list.get(3))) && isClosingBracket(list.get(4))){System.out.println("Debug--4");
                        Method.run(list, 1);
                    }
            ...
            for(int i=0; i<list.size();i++){
                ...
                if( isClosingBracket(list.get(i)) ){
                    ...
                    if(open>0){
                        ...
                        else if(isMethodName(token)||isUnaryPreOperator(token)||isNumber(token)){
                            List<String> l = list.subList(open-1, i+1); int siz = l.size();
                            String input;
                            if(isStatsMethod(token)){
                                /**
