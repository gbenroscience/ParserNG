...
public final class UnaryPreOperator extends Operator implements Validatable{
   ...
   private final Precedence precedence;
     ...
     *
     * @param name The name that identifies this UnaryPreOperator object
     * @param function The MathExpression object that contains this object
     */
    ...
    try{
          ...
          &&!isAssignmentOperator(scan.get(index-1))
            ...
            "Mathron Does Not Allow "+getName()+" To Combine The Function Members \""+scan.get(index-1)+"\" And \""+scan.get(index)+"\""+
                        " As You Have Done."+
            "Mathron Error Detector For Pre-number operators!" );
             correct = false;
             scan.clear();
         }//end if
       //specify valid tokens that can come after a pre-number operator
         if(!isOpeningBracket(scan.get(index+1))&&!isNumber(scan.get(index+1))
           &&!isUnaryPreOperatorORDefinedMethod(scan.get(index+1))&&
          !isVariableString(scan.get(index+1))&&!isNumberReturningStatsMethod(scan.get(index+1))
          &&!isLogToAnyBase(scan.get(index+1))&&!isAntiLogToAnyBase(scan.get(index+1))
          ){
          util.Utils.logError(
 ...
 * @param index the index of the operator in the scanner output
 * of the MathExpression object.
 */
