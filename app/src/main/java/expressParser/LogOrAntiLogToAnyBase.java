...
private int index;
    ...
    public LogOrAntiLogToAnyBase(String name,int index,ArrayList<String>scan) {
        super((isLogOrAntiLogToAnyBase(name))?name:""); 
        this.index=index;
   ...
   if(!isNumber(scan.get(index-1))&&!isBinaryOperator(scan.get(index-1))&&
           !isVariableString(scan.get(index-1))&&!isBracket(scan.get(index-1))
            ...
            "Mathron Does Not Allow "+getName()+" To Combine The Function Members \""+scan.get(index-1)+"\" And \""+scan.get(index)+"\""+
                        " As You Have Done."+
            "Mathron Error Detector For LogOrAntiLogToAnyBase Operators!" );
             correct=false;  scan.clear();
         }//end if
       //specify valid tokens that can come after a LogOrAntiLogToAnyBase operator
         if(!isNumber(scan.get(index+1))&&!isOpeningBracket(scan.get(index+1))&&!isNumberReturningStatsMethod(scan.get(index+1))
            &&!isUnaryPreOperator(scan.get(index+1))
            &&!isLogToAnyBase(scan.get(index+1))&&!isAntiLogToAnyBase(scan.get(index+1))&&!isStatsMethod(scan.get(index+1))
                 ){
            util.Utils.logError(
