            ...
            try{
                if(isPower(scan.get(i))){
                    ...
                    if(isOpeningBracket(scan.get(i+1))){
                        ...
                        if(isNumber(scan.get(j+1))|| ( isVariableString(scan.get(j+1))&&!isOpeningBracket(scan.get(j+2)) )  ){
                            isSimpleNumberOperandCase=true;
                        }
                    }
                    else if(isNumber(scan.get(i+1))|| ( isVariableString(scan.get(i+1))&&!isOpeningBracket(scan.get(i+2)) ) ){
                        isSimpleNumberOperandCase=true;
                    }
//end of checking the scenario.The boolean isSimpleNumberOperandCase contains the scenario.
                    ...
                    if(isSimpleNumberOperandCase){//cos(x)^2
                        ...
                        if(isNumber(scan.get(i-1))|| ( isVariableString(scan.get(i-1))&&!isOpeningBracket(scan.get(i)) )  ){
                            scan.add(i-1,"(");
                            ...
                            if( index-1 > 0 && isMethodName(scan.get(index-1))){
                                continue;
                            }
                            else{
                                scan.add(index,"(");
                        ...
                        if(isNumber(scan.get(i+1))|| ( isVariableString(scan.get(i+1))&&!isOpeningBracket(scan.get(i+2)) ) ){
                            scan.add(i+2,")");
...
//end of checking the scenario.The boolean isSimpleNumberOperandCase contains the scenario.
                    ...
                    if(!isSimpleNumberOperandCase){
                        ...
                        else if( isUnaryPreOperator(scan.get(i+1))||( isDefinedMethod(scan.get(i+1))&&isOpeningBracket(scan.get(i+2)) ) ){
                            int j=i+1;
                            while( isUnaryPreOperator(scan.get(j))||( isDefinedMethod(scan.get(j))&&isOpeningBracket(scan.get(j+1)) ) ){
                                ++j;
                            }
                            if(isNumber(scan.get(j))||isVariableString(scan.get(j))){
                                scan.add(j+1, ")");
                                ++i;
                            }
                        ...
                        else if(isNumberReturningStatsMethod(scan.get(i+1))||isLogOrAntiLogToAnyBase(scan.get(i+1))){
