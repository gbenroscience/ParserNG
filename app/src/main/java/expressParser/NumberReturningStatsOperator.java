...
public class NumberReturningStatsOperator extends Operator implements Validatable{
    ...
    public NumberReturningStatsOperator(String name,int index,ArrayList<String>scan) {
        super((isNumberReturningStatsMethod(name)||isLogOrAntiLogToAnyBase(name))?name:"");
        this.index=index;
...
boolean correct = true;
       ...
       &&!isLogicOperator(scan.get(index-1))&&!isAssignmentOperator(scan.get(index-1))
                 &&!isAntiLogToAnyBase(scan.get(index-1))
                 &&!isLogToAnyBase(scan.get(index-1))&&!isUnaryPreOperator(scan.get(index-1))
          ...
          util.Utils.logError(
            "Mathron Does Not Allow "+getName()+" To Combine The Function Members \""+scan.get(index-1)+"\" And \""+scan.get(index)+"\""+
                        " As You Have Done."+
            "Mathron Error Detector For Number Returning Stats operators!" );
             correct = false;
             scan.clear();
         }//end if
