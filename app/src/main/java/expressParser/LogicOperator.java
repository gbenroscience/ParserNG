 ...
 * LogicOperator object belongs to.
 */
 ...
 * that this LogicOperator object exists in.
 ...
 &&!isClosingBracket(scan.get(index-1)) )
         ...
         util.Utils.logError(
            "Mathron Does Not Allow "+getName()+" To Combine The Function Members \""+scan.get(index-1)+"\" And \""+scan.get(index)+"\""+
                        " As You Have Done."+
            "Mathron Error Detector For Logic operators!" );
             correct = false;
             scan.clear();
       ...
       //specify valid tokens that can come after a logic operator
         if(!isNumber(scan.get(index+1))&&!isVariableString(scan.get(index+1))&&
             ...
             !isUnaryPreOperatorORDefinedMethod(scan.get(index+1))&&!isNumberReturningStatsMethod(scan.get(index+1))
