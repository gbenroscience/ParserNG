...
public static final Variable PI = new Variable("pi", Maths.PI(), true);
    ...
    /**
 * The last answer variable used for flexibility on
 ...
 * @param constant the nature of the Variable object whether it is modifiable
 * or not.
 *  If constant = true , then the Variable object represents
 *  a constant, whose value cannot be altered.Else,it represents a Variable
 * object whose value can change.
 */
...
/**
 * sets the nature of the Variable object
 * to either Variable or constant
 * @param constant = true if the Variable object
 * is to be changed into a constant
 * and constant = false if the Variable object is
 * to be changed into a Variable object
 */
...
/**
 * If the name is a string of alphabets and is not an
 * operator name, then it is a valid variable name.
 *
...
public static boolean isVariableString(String var){
 ...
 else if( var.equals("var")||var.equals("const")||purifier(var).equals("")){
     return false;
 }//end else if
 ...
 * @return true if the variable is one that is already defined by the parser for its own purposes.
 * An example of such a variable is any constant parameter recognized by the parser, i.e to which
 * the parser has attached a meaning already. Also any parameter such as the "ans" parameter which returns the
 * last value calculated by the parser is regarded by the parser to be a system variable.
 *
 */
 ...
 *
 * @param str the name of the String variable
 * @return true if the variable is a constant one that is already defined by the parser for its own purposes.
 * An example of such a constant is any constant defined by the parser e.g PI and so on.
 *
 */
...
public boolean isSimpleVar(){
   return ( (this.name.length()==1)&&(isUpperCaseWord(name)||name.equals("θ"))    );
}
 ...
 *              To support this, the user should place a space between the
 *              value and the units.
 *
 */
...
/**
 *
 * @param var the String property of the variable.This method returns the index of the Variable
 * object that has this String property.
