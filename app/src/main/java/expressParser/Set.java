...
//the maximum finder algorithm begins
    ...
    /**
     *
     * @return displays the output of the result method as the sorting process proceeds.
     */
     ...
     * @return a the solution to the system of equations represented by the number list
     */
    ...
    /**
     * The input list is such that:
     * The first 2 entries specify the number of rows and columns.
     * The remaining entries are the Matrix's entries.
     * @return a number list which represents the row-reduced echelon Matrix that the entries reduce to.
     */
     ...
     * The first 2 entries specify the number of rows and columns.
     * The remaining entries are the Matrix's entries.
     * @return a number list which represents the triangular Matrix that the entries reduce to.
     */
    ...
    public double median(){
        double median = 0;System.out.println("sort: "+data);
...
//check if the random command came with only one argument
    ...
    /**
     * Finds the numerical derivative of a Function which has been pre-defined in
     * the Workspace.
     * @return the derivative at the specified value of the horizontal coordinate.
     */
...
*/
    ...
    /**
     * Finds the zero of a Function which has been pre-defined in
     * the Workspace.
    ...
    public String integrate(){
        ...
        boolean has2NumberArguments=false;
        if(validNumber(data.get(sz-1)) && validNumber(data.get(sz-2)) && validNumber(data.get(sz-3))){
    ...
    /**
     *
     * @return the determinant or {@link Double#NaN} if the determinant does not exist.
     */
    ...
    /**
     * The list must have been originally supplied:
     * matrix_mul(A,B) {where A and B are matrices}
     *  or matrix_mul(A,k) {where A is a matrix and k is a scalar}
     * It multiplies them out and returns the result as a list.
    ...
    public Matrix multiplyMatrix(){
        ...
        if(data.size() == 2){
            ...
            if( isVariableString(token) ){
                if( isVariableString(nextToken) ){
    ...
    /**
     * The list must have been originally supplied:
     * matrix_div(A,B) {where A and B are matrices}
     *  or matrix_div(A,k) {where A is a matrix and k is a scalar}
     * It divides them out and returns the result as a list.
     * @return  a {@link Matrix} containing the matrix division (scalar or vector)
     * depending on the second argument.
     */
    ...
    /**
     * The list must have been originally supplied:
     * matrix_add(A,B) {where A and B are matrices}
     * It adds them out and returns the result as a list.
     * @return a {@link Matrix}containing the matrix addition (scalar or vector).
     */
    ...
    /**
     * The list must have been originally supplied:
     * matrix_sub(A,B) {where A and B are matrices}
     * It subtracts them out and returns the result as a list.
    ...
    /**
     * The list must have been originally supplied:
     * matrix_pow(A,n) {where A is a Matrix and n is an integer. If n is a double, it is rounded down to the nearest int.}
     * It raises the Matrix to the power of the second argument and returns the result as a list.
    ...
    public Matrix powerMatrix(){
        ...
        if(data.size() == 2){
            ...
            if( isVariableString(token) ){
                if( isVariableString(nextToken) || validNumber(nextToken) ){
                    Function f = FunctionManager.lookUp(token);
                    double pow = isVariableString(nextToken) ? Double.parseDouble( VariableManager.lookUp(nextToken).getValue() ) : Double.parseDouble(nextToken);
    ...
    /**
     * The list must have been originally supplied:
     * transpose(A) {where A is a Matrix}
     * It transposes A and returns the result as a list.
