...
String numbeforepoint="";
 ...
 catch(NumberFormatException numa){ checkmethodgetNumbersBeforePoint=0;  }
...
double f=0;double f1=0;
  ...
  catch(NullPointerException nola){  h=STRING.reverse("PLEASE ENTER A NUMBER");  }
  catch(NumberFormatException no){   h=STRING.reverse("NUMBER TOO LARGE");   }
...
String h="";//variable that will store the output of this method
    ...
    num=STRING.purifier(num);num_base=STRING.purifier(num_base);//remove all white spaces
...
h=String.valueOf(H);
    ...
    catch(NullPointerException nol){h="SYNTAX ERROR at null";}
    catch(IndexOutOfBoundsException ind){h="SYNTAX ERROR at ind";ind.printStackTrace();}
    catch(NumberFormatException numa){h="SYNTAX ERROR at numa";numa.printStackTrace();}
