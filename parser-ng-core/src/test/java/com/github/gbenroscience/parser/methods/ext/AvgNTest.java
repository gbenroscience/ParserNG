package com.github.gbenroscience.parser.methods.ext;

import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;
import com.github.gbenroscience.parser.MathExpression;

public class AvgNTest {

    @Test
    void zeroZero() {
        MathExpression me = new MathExpression("avgN(0,0)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(1,0)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(2,0)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(3,0)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(0,1)");
        Assertions.assertEquals(1, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(0,1,2)");
        Assertions.assertEquals(1.5, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(0,1,2,3)");
        Assertions.assertEquals(2, Double.parseDouble(me.solve()));
    }

    @Test
    void decimalsInCountWorksAsHould() {
        MathExpression me;
        me = new MathExpression("avgN(0.4, 1, 2, 3, 4, 15)");
        Assertions.assertEquals(5, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(0.6, 1, 2, 3, 4, 15)");
        Assertions.assertEquals(3, Double.parseDouble(me.solve()));
    }

    @Test
    void limitWorks() {
        MathExpression me = new MathExpression("avgN(1,45,1,55,1000)");
        Assertions.assertEquals(50, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(2,45,1,1000,55)");
        Assertions.assertEquals(50,Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(2,45,1,1000,80,55)");
        // the avgN default size DEFAULT_AVGN_MISIZE is two, thus 45,1,1000,80,55 -> 1,45,55,80,1000-> 45,55,70 => 180/3
        Assertions.assertEquals(60, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(3,45,1,1000,80,55)");
        Assertions.assertEquals(60, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(3,45,1,1000,80,55,0)");
        Assertions.assertEquals(50, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(3,45,1,1000,80,55,0,0)");
        // the avgN default size DEFAULT_AVGN_MISIZE is two, thus 45,1,1000,80,55,0,0 -> 0,0,1,45,55,80,1000-> 1,45,55=> 101/3
        Assertions.assertEquals(33.66666667, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(3,45,1,1000,80,55,0,0,0)");
        Assertions.assertEquals(23, Double.parseDouble(me.solve()));
    }

    @Test
    void testEmpty() {
        MathExpression me;
        me = new MathExpression("avgN(1+1)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(1)");
        Assertions.assertEquals(0, Double.parseDouble(me.solve()));
    }

    @Test
    void testExpres() {
        MathExpression me;
        me = new MathExpression("avgN(0+1, 0+1)");
        Assertions.assertEquals(1, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(avgN(0,1), avgN(0,1))");
        Assertions.assertEquals(1, Double.parseDouble(me.solve()));
    }

    @Test
    void avgNFromExpressions() {
        MathExpression me;
        me = new MathExpression("avgN(0,4+0,2+0)");
        Assertions.assertEquals(3, Double.parseDouble(me.solve()));
        me = new MathExpression("avg(4+0,2+0)");
        Assertions.assertEquals(3, Double.parseDouble(me.solve()));
        me = new MathExpression("avg(0,3/2,2/3)");
        Assertions.assertEquals(0.7222222222, Double.parseDouble(me.solve()));
        me = new MathExpression("avg(round(0.5),round(0.5))");
        Assertions.assertEquals(1, Double.parseDouble(me.solve()));
        me = new MathExpression("avgN(0,round(0.5),round(0.5))");
        Assertions.assertEquals(1, Double.parseDouble(me.solve()));
        me = new MathExpression("round(avg(0,3/2,2/3))");
        Assertions.assertEquals(1,Double.parseDouble(me.solve()));
    }
}
