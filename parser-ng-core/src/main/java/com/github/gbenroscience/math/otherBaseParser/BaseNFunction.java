...
public class BaseNFunction extends MathExpression {
     ...
     * initialized and then an expression to evaluate. e.g. var x =-12; var y
     * =x+1/12; const x1,x2,x3=10; var z =sin(3x-1)+2.98cos(4x);cos(3x+12); The
     * last expression is function to be evaluated and it is always without any
     * equals sign and may or may not end with a semicolon.
     * @param baseOfOperation The number base that objects of this class will
     * use in computations.
     */
     ...
     *
     * @return An {@link ArrayList} containing the indices of all numbers in the
     * scanner.
     */
        ...
        if (isCorrectFunction()) {
            ...
            for (int i = 0; i < numIndices.size(); i++) {
                ...
                if (isVariableString(mathObject) && !handledVariables.contains(mathObject)) {
                    ...
                    setValue(mathObject, Double.parseDouble(value));
