...
//²
        ...
        /**
         * The scanner of this project does not separate number.concat(variable)
         * tokens but requires them to be separated by the * operator.
         *
         * Our work here however requires this feature so we enable it with this
         * for loop. This loop takes the unscanned number.concat(variable)
         * tokens and separates them into a number token, a * token and a
         * variable token.
         */
...
//validates variable..power..exponent arrangement
        ...
        //The second logic turns say 2x^2 to 2x²,..2x^1 to 2x and 2x^0 to 2.
        for (int i = 0; i < scanner.size(); i++) {
            ...
            try {
                ...
                if (isVariableString(scanner.get(i - 1)) && isPower(scanner.get(i)) && validNumber(scanner.get(i + 1))
                        && Double.valueOf(scanner.get(i + 1)) != 0.0 && Double.valueOf(scanner.get(i + 1)) != 1.0 && Double.valueOf(scanner.get(i + 1)) != 2.0) {
                    setValid(false);
                    break;
                    ...
                    if (Double.valueOf(scanner.get(i + 1)) == 0.0) {
                        ...
                        scanner.set(i, "");
                        scanner.set(i + 1, "");
                    } else if (Double.valueOf(scanner.get(i + 1)) == 2.0) {
                        scanner.set(i, "²");
                        scanner.set(i + 1, "");
                    }
...
//0x^2+0x+0 to the equation.
    ...
    /**
     * Handles consecutive strings of plus and minus operators and simplifies
     * them, replacing them with their single equivalent in plus and minus
     * terms.e.g +++---+ is equivalent to a - and --++-- is equivalent to a +.
     */
     ...
     * @return the first index of occurrence of a letter, and -1 if no letter of
     * the English alphabet is present.
     */
...
-2.3E-2x-2.09y+3.01E-2z=1.39:
        ...
        for (int i = 0; i < scanner.size(); i++) {
            ...
            try {
                ...
                if (st.equalsIgnoreCase("E")) {
                    ...
                    /**
                     * 2 scenarios are possible: 1.
                     * number,E|e,+|-,number,variable,+|-....e.g 2.089E-5x,or
                     * 2.089E+5x OR: number,E|e,number,variable,+|- 0.089E5x
                     *
                     */
                    /**
         ...
         * Our work here does not require brackets and the multiplication symbol
         * so we remove them.
         */
        remover.add("");
        remover.add("(");
    ...
    /**
     * When the situation x+2y+z=9; is met, this method will convert it into
     * 1.0x+2y+z=9;
     */
    ...
    public void validateChars() {
        ...
        for (int i = 0; i < scanner.size(); i++) {
            try {
                if (!isSquare(scanner.get(i)) && !validNumber(scanner.get(i)) && !isVariableString(scanner.get(i))
        ...
        for (int i = 0; i < scanner.size(); i++) {
            try {
                if (isVariableString(scanner.get(i))) {
                    ...
                    if (!isSquare(scanner.get(i + 1)) && !scanner.get(i + 1).equals("+") && !scanner.get(i + 1).equals("-")
    ...
    /**
     * Checks the left and right of a number to see if the appropriate items are
     * the ones there. It is a syntax error if an invalid item is found there
     */
    ...
    public void recognizeCompoundVariables() {
        ...
        for (int i = 0; i < sz; i++) {
            try {
                ...
                String nextTkn = i+1 < sz ? scanner.get(i+1) : null;
                
                if (isVariableString(tkn) && (nextTkn != null && isSquare(nextTkn)) ) {
                    if (!vars.contains(scanner.get(i))) {
                        vars.add(tkn + "²");
                        vars.add(tkn);
                    }
                    scanner.set(i, tkn + nextTkn);
                    scanner.set(i + 1, "");
                }
            }//end try
            catch (IndexOutOfBoundsException indexErr) {
        ...
        //end for
//The ArrayList vars will contain all variables and their squared versions that may occur in the input.
//variable var will contain the name of the last variable that is encountered in the loop.
        freeWhiteSpaces();

        String var = vars.isEmpty() ? null : vars.get(0).substring(0, vars.get(0).indexOf("²"));

        if (var != null) {
//We need to be sure that the input consists of just one variable and its squared version.
            for (int i = 0; i < scanner.size(); i++) {
                String str = scanner.get(i);
                if (isVariableString(str) || isVariableString(str.substring(str.length() - 1))) {
                    /**
                     * The variable must be equal to its squared version without
                     * the squaring symbol or it must be equal to any other
                     * variable in the system. Else,invalidate the system,
                     */
                    if (!var.equals(str.substring(str.length() - 1)) && !var.equals(str)) {
                        setValid(false);
                        break;
                    }
                }
    ...
    /**
     *
     * This method initializes the coefficients field with the coefficient of
     * Xsquared at index 0 the coefficient of X at index 1 the coefficient of
     * the constant term at index 2
     *
     */
    ...
    public static void main(String args[]) {
        QuadraticExpressionParser parser = new QuadraticExpressionParser("3.00098E-2x-5x^2=-9+2");
